...
- All 40 redacted messages contain the policy placeholders where the source contained a match; the forbidden fixture strings (`4539 1488 0343 6467`, `4539148803436467`, `CVV 441`, `cvv 441`) never appear in any `redacted_text`, `replacement_placeholder`, `risk_flags`, `LinkageResult`, or CLI stdout — asserted by `test_m11_pan_and_cvv_bypass_sensitive`, `test_ingest_command_prints_sanitized_summary`, and by the ingestion error test `test_ingestion_error_does_not_echo_row_body`.
